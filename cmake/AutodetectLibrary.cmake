...
macro(autodetect_library FRIENDLY NAME PACKAGE PKGCONFIG HEADER LIBRARY)
    ...
    if (${NAME}_FOUND)
        ...
        # Fix incomplete detection for Visual Studio generators
        if (MSVC AND CMAKE_CONFIGURATION_TYPES)
            string(FIND "${${NAME}_LIBRARIES}" "optimized;" OPTIMIZED)
            if (OPTIMIZED LESS 0)
                string(FIND "${${NAME}_LIBRARY}" "debug/" DEBUG)
                if (DEBUG LESS 0)
                    ...
                    string(REPLACE "debug/" "" ${NAME}_LIBRARY_RELEASE "${${NAME}_LIBRARY}")
                endif (DEBUG LESS 0)
                set(${NAME}_LIBRARIES "optimized" "${${NAME}_LIBRARY_RELEASE}" "debug" "${${NAME}_LIBRARY_DEBUG}")
            endif (OPTIMIZED LESS 0)
