...
- (void)keyDown:(NSEvent *)event
{
	...
	if (EditBoxInGlobalFocus()) {
		if ([ self internalHandleKeycode:event.keyCode unicode:unicode_str.front() pressed:YES modifiers:event.modifierFlags ]) {
			[ self interpretKeyEvents:[ NSArray arrayWithObject:event ] ];
		}
	} else {
...
- (void)flagsChanged:(NSEvent *)event
{
	...
	for (unsigned int i = 0, bit = NSAlphaShiftKeyMask; bit <= NSCommandKeyMask; bit <<= 1, ++i) {
		...
		if (currentMask && currentMask != newMask) { // modifier up event
			...
			if (bit == NSAlphaShiftKeyMask) [ self internalHandleKeycode:mapping[i] unicode:0 pressed:YES modifiers:newMods ];
			...
			/* If this was Caps Lock, we need some additional voodoo to make SDL happy (is this needed in ottd?) */
			if (bit == NSAlphaShiftKeyMask) [ self internalHandleKeycode:mapping[i] unicode:0 pressed:NO modifiers:newMods ];
		}
	}
