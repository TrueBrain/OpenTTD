...
/** @file sdl2_opengl_v.cpp Implementation of the SDL2 video driver. */
...
void VideoDriver_SDL_OpenGL::MakeDirty(int left, int top, int width, int height)
{
...
bool VideoDriver_SDL_OpenGL::AllocateBackingStore(uint w, uint h, bool force)
{
...
{
	if (this->dirty_rect.bottom == 0 && _cur_palette.count_dirty == 0) return;
...
void VideoDriver_SDL_OpenGL::DrawMouseCursor()
{
	PerformanceMeasurer framerate(PFE_VIDEO);
